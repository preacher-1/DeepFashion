...
if __name__ == '__main__':
    ...
    for epoch in range(20):
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            if i % 100 == 0:
                print(
                    f"Epoch {epoch}, Batch {i}, Loss NVAE: {loss_nvae.item():.4f}")
        ...
        print(f"Epoch {epoch}, Test Loss NVAE: {test_loss_nvae:.4f}")
        print(
            f"Epoch {epoch}, Test Accuracy NVAE: {100 * correct_nvae / len(test_dataset):.2f}%")

    # Plot loss and accuracy curves
    plt.subplot(2, 1, 1)
    # plt.plot(resnet_train_loss, label='ResNet18 Train Loss')
    plt.plot(nvae_train_loss, label='NVAE Train Loss')
    # plt.plot(resnet_test_loss, label='ResNet18 Test Loss')
    plt.plot(nvae_test_loss, label='NVAE Test Loss')
    plt.legend()
    plt.title('Loss Curves')

    plt.subplot(2, 1, 2)
    # plt.plot(resnet_test_acc, label='ResNet18 Test Accuracy')
